...
module.exports = function (app) {
	...
	// Admin section
	app.get('/interviews', auth.validated, function (req, res) {
		// the group id of the user.. so we can show only the relevant interviews
		var group_id = req.session.user.group;
		var user_id = req.session.user.id;

		models.Saves.find({}).where('user_id').equals(user_id).exec(function(err, saves) {
			if (err) {
				console.log(err);
				throw err;
			} 
			// send the output to the view
			res.render('interviews/layout', { 
	...
	app.post('/interviews/ltf_login', function (req, res) {
		if (req.method === 'POST') {
	...
	app.post('/interviews/ltf_register', function (req, res) {
		// sanitize the inputs from the form
	...
	app.all('/interviews/reset', function (req, res) {
		var view = function (data) { 
			...
			res.render('interviews/auth/forgot', load);
		};
	...
	// send the user a password
	app.all('/interviews/reset/:reset_token', [auth.validateResetToken], function (req, res) {
		...
		function view (data) { 
			...
			res.render('interviews/auth/reset', load);
		}
	...
	app.get('/interviews/post/status',  function (req, res) {
		if (req.session.user) {
	...
	app.get('/interviews/post/id',  function (req, res) {
		if (req.session.user) {
	...
	app.get('/interviews/logout', auth.validated, function (req, res) {
		req.session.user = {};
