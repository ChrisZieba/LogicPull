...
exports.common = {
	...
	// if not format is given the default is "MM/DD/YYYY"
...
exports.interview = {
	 ...
	 *
	 * @answer {variable} The varibale which stores the checkbox id's
	...
	/* this takes the answer as a string, and the id
	// TODO - this function is slow!!! - it loops through every question and field because we do not know where the field is
	fieldLabel: function (name, id, index) {

		var qid;

		index = (typeof index !== 'undefined') ? parseInt(index, 10) : null;

		// first check if the name is defined, and if a checkbox, radio, or drop down
		for (qid in data) {
			if (data.hasOwnProperty(qid)) {
				// go through each filed until we find the one we need
