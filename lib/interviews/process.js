						...
						fs.writeFile(base_location + tmp_directory + tmp, stylesheet, function (err) {
							...
							cmd.on('exit', function (code) {
								if (code !== 0) {
									...
									callback('The stylesheet [' + tmp + '] could not be processed. Check that Apache FOP is installed correctly and that the path to the binary (located in config.js) is correct.');
								} else {
