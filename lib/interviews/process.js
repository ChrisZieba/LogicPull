...
function outFilename (interview, name ,type, client, date, count) {
	...
	return interview.toUpperCase() + name.toUpperCase() + monthNames[date.getMonth()].toUpperCase() + '_' + date.getDate() + '_' + date.getFullYear() + '_' + count + '_' + Date.now() +'.' + type;
}
 ...
 *
 */
exports.output = function (doc, master, progress, base_location, app, socket_callback) {
	"use strict";
						...
						fs.writeFile(base_location + tmp_directory + tmp, stylesheet, function (err) {
							...
							cmd.on('exit', function (code) {
								if (code !== 0) {
									...
									callback('The stylesheet [' + tmp + '] could not be processed');
								} else {
...
exports.email = function (email, interview_name, base, data, complete, deliverables, client, app, socket_callback) {
	...
	async.waterfall([function (callback) {
		...
		// check to see if we want to email notifications out, and its not disabled in configure
