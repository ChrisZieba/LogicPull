...
function build (question, master, logic_debug, type, fields, data, answers) {
  ...
  var date_pickers = [];
  var max, min, values, field, label_class, input_name, input_type, input_label, input_default, input_validation, input_values, value, button, button_type, button_destination, button_text, descending, size, line;

  // Starts the question
  output.push('<div class="col-lg-12 contents">'); 
  output.push('<div class="well">'); 
  ...
  if (question.fields) {
    ...
    for (var i = 0; i < question.fields.length; i+=1) {
      ...
      var class_size = "col-md-12";
      var rows = "12";
      // Do not want to vertically seperate textareas into smaller columns
      ...
      output.push('<div class="' + class_size + '">');
      ...
      if (input_name && input_type) {
        ...
        output.push('<div id="' + input_name + '-var-container" class="var-container form-group">');
        ...
        switch (input_type) {
          case 'text':
            ...
            if (value !== null && typeof value !== undefined && value !== '') {
              output.push('<input type="text" value="' + value + '" name="' + input_name + '" size="40" class="form-control notranslate" />');
            } else {
              output.push('<input type="text" value="" name="' + input_name + '" size="40" class="form-control notranslate" />');
            }
          ...
          case 'textarea':
            ...
            output.push('<ul id="' + input_name + ':' + input_type + '" class="id-field">');
            output.push('<li>');
            output.push('<textarea name="' + input_name + '" cols="34" rows="' + rows + '" class="form-control notranslate" data-min-length="' + field.validation.min_length + '" data-max-length="' + field.validation.max_length + '">' + value + '</textarea>');
            output.push('<div class="character-count"></div>');
            output.push('</li>');
            output.push('</ul>');
            break;
          case 'number':
          ...
          case 'date':
            ...
            output.push('<li>');
            output.push('<div class="input-group">');
            output.push('<input type="text" id="' + input_name + '_picker" name="' + input_name + '" size="14" class="form-control notranslate" readonly="readonly" placeholder="Click to select" />');
            output.push('<span class="input-group-addon"><i class="fa fa-calendar"></i></span>');
            output.push('</div>');
            output.push('</li>');
            output.push('</ul>');
          ...
          case 'text_dropdown':
            if (input_values) {
              ...
              output.push('<li>');
              output.push('<select name="' + input_name + '" class="form-control">');

              value = helpers.getAnswer(input_name, fields, type, input_default, answers);

              // if we are going back, the value is the previous answer, if going forward, it is just the default
              if (value !== null && typeof value !== undefined && value !== '') {
                output.push('<option value="" class="notranslate">--- Please select a value ---</option>');
              ...
              // no default set
                ...
                output.push('<option value="" selected="selected" class="notranslate">--- Please select a value ---</option>');
  ...
  if (question.buttons) {
    output.push('<div class="row question-footer">');
      for (var r = 0; r < question.buttons.length; r+=1) {
        ...
        // Make the first letter upercase
        button_text = button_type.charAt(0).toUpperCase() + button_type.slice(1);
        ...
        if (button_type === "exit") {
          output.push('<div id="exit-interview" class="' + button_type + '-button pull-right btn btn-danger notranslate" data-qid="' + question.qid + '" data-destination="' + button_destination + '">' + button_text + '</div>');
        } else if (button_type === "finish") {
          output.push('<div id="finish-interview" class="' + button_type + '-btn btn btn-success pull-right notranslate" data-loading-text="Loading..." autocomplete="off" data-qid="' + question.qid + '" data-destination="' + button_destination + '">' + button_text + '</div>');
        } else {
          output.push('<div class="' + button_type + '-button button btn btn-primary pull-right notranslate" data-loading-text="Loading..." autocomplete="off" data-qid="' + question.qid + '" data-destination="' + button_destination + '">' + button_text + ' <i class="fa fa-arrow-circle-o-right"></i></div>');
        }
      }
    output.push('</div>');
    output.push('<div class="clear"></div>');
  ...
  if (question.learn_more) {
    if (question.learn_more.body) {
      lm.push('<div class="contents well">');
  ...
  output.push('<div class="col-lg-12 contents">');
    output.push('<div class="well">');
      ...
      if (deliverables.length !== 0) {
        output.push('<div class="text"><h1>Success!</h1><br><p>Your documents were created successfully. Enter your email to receive them. If you are a registered user, you can also login to the /admin area to download them at a later time.</p><br><p><i>We will <strong>not</strong> store your email or use it for anything other than sending you these documents.</i></p></div>'); 
      } else {
        output.push('<div class="text"><h1>Oops!</h1><br><p>We weren\'t able to generate any documents for you. There are no deliverables set for this interview!</p><br><br></div>'); 
      }
      ...
      output.push('<div id="q-final-var-container" class="var-container form-group">');
        output.push('<div class="var-label required">Email</div>');
        output.push('<ul id="q-final:text" class="id-field">');
          output.push('<li>');
            output.push('<input type="text" value="" name="q-final" size="40" class="form-control" />');
          output.push('</li>');
        output.push('</ul>');
      output.push('</div>');

      output.push('<div class="row question-footer">');
        output.push('<div id="exit-interview" class="btn btn-default pull-right notranslate">Exit</div>');
        output.push('<div id="complete-interview" class="btn btn-success pull-right notranslate" data-loading-text="Loading..." autocomplete="off">Send Email</div>');
      output.push('</div>');
 ...
 *
 */
function logic (qid, question, data, event, master, debug, show_name, progress, skip_goto, destination) {
  "use strict";
              ...
              if (act.value !== 'none') {
                ...
                if (act.if === condition) {
                  ...
                  if (event === 'before') {
                    ...
                    get_logic = logic(qid, data[qid], data, event, master, debug, true, progress, null);
    ...
    if (old_qid === qid) {
      if (question.buttons) {
        for (var p = 0; p < question.buttons.length; p+=1) {
          if (question.buttons[p].type === 'continue' && question.buttons[p].destination !== 'none') {
            qid = question.buttons[p].destination;
            if (destination && qid === destination) {
              break;
            }
          } 
        }
...
exports.start = function (qid, question, data, master, progress, state) {
  ...
  var before_logic = logic(qid, question, data, 'before', master, [], true, progress, null);
 ...
 *
 */
exports.next = function (qid, question, data, master, fields, progress, state, destination) {
  "use strict";

  // if we hit a 'GOTO in the before logic, exit the logic and return the qid of the new question
  var after_logic = logic(qid, question, data, 'after', master, [], false, progress, false, destination);
  ...
  var before_logic = logic(after_logic.qid, data[after_logic.qid], data, 'before', after_logic.vars, after_logic.debug, true, after_logic.progress, null);
...
exports.distance = function (questions, start, callback) {
  ...
  // this takes the dot graph generated above and creates a dot file with tall the positions
...
exports.done = function () {
  ...
  output.push('<div class="col-lg-12 contents">'); 
    output.push('<div class="well">'); 
      output.push('<div class="text"><h1>Finished!</h1><br><p>The interview is now complete, and all documents have been processed successfully.</p></div>'); 
      output.push('<div class="row question-footer">');
        output.push('<div id="exit-interview" class="btn btn-default pull-right notranslate">Exit</div>');
      output.push('</div>');
    output.push('</div>'); 
 ...
 *
 */
exports.saves = function (saved, admin) {
  "use strict";
  ...
  if (saved.length === 0) {
    output.push('<div class="alert alert-warning">You have no saved interviews.</div>');
  } else {
    output.push('<p>If you choose to load an existing interview, <strong>your current interview will be lost unless you save it first</strong>.</p><br>');
    output.push('<table class="table table-bordered">');
    output.push('<thead>');
    ...
    for (var i = 0; i < saved.length; i+=1) {
      ...
      output.push('<td><div class="">' + saved[i].note + '</div></td>');
      ...
      output.push('<td>');
      output.push('<button id="partial-'+ saved[i].id +'" title="Continue this interview" data-loading-text="Loading..." autocomplete="off" class="btn btn-primary btn-sm partial-sav-int" style="margin: 0 2px">Load</button>');

      if (admin) {
        output.push('<button id="partial-process-'+ saved[i].id +'" title="Process documents" data-loading-text="Loading..." autocomplete="off" class="btn btn-primary btn-sm partial-process-int" style="margin: 0 2px">Process</button>');
      }
...
exports.error = function (err) {
  ...
  if (typeof err === "undefined") {
    err = "Sorry, but something seems to have gone wrong.";
  ...
  output.push('<div class="col-lg-12 contents">'); 
    output.push('<div class="well">'); 
      output.push('<div class="text"><h1>Oops!</h1><br><p>' + err + '</p></div>'); 
