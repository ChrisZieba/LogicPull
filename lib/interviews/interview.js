...
exports.saves = function (saved) {
  ...
  if (saved.length === 0) {
    ...
    for (var i = 0; i < saved.length; i+=1) {
      ...
      output.push('<td><button id="partial-'+ saved[i].id +'" title="Continue this interview" data-loading-text="Loading..." autocomplete="off" class="btn btn-primary btn-sm partial-sav-int">Load</button>');
      output.push('</td>');
      output.push('</tr>');
    }
