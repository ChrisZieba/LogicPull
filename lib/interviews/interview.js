	...
	if (question.buttons) {
		for (var r = 0; r < question.buttons.length; r+=1) {
			...
			if (button_type === "exit") {
				...
				output.push('<div id="exit-interview" class="' + button_type + '-button pull-right notranslate">' + question.qid + '</div>'); 
			} else if (button_type === "finish") {
				// removes the button event listener
				output.push('<div id="finish-interview" class="' + button_type + '-btn button btn btn-success pull-right notranslate">' + question.qid + '</div>'); 
			} else {
				output.push('<div class="' + button_type + '-button button btn btn-lg btn-primary pull-right notranslate" data-loading-text="Loading..." autocomplete="off" data-qid="' + question.qid + '">' + button_type.charAt(0).toUpperCase() + button_type.slice(1) + ' <i class="fa fa-arrow-circle-o-right"></i></div>');
			}
		}
		output.push('<div class="clear"></div>');
	}
...
exports.saves = function (saved) {
	...
	if (saved.length === 0) {
		...
		output.push('<p>If you choose to load an existing interview, your current interview will be lost, <strong>unless you save it first</strong>.</p>');
		...
		for (var i = 0; i < saved.length; i+=1) {
			...
			output.push('<td><button id="partial-'+ saved[i].id +'" title="Continue this interview" class="btn btn-primary btn-sm">Load</button>');
			output.push('</td>');
			output.push('</tr>');
		}
