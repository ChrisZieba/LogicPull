			...
			if (input_name && input_type) {
				...
				switch (input_type) {
					...
					case 'textarea':
						...
						output.push('<ul id="' + input_name + ':' + input_type + '" class="id-field">');
						output.push('<li>');
						output.push('<textarea name="' + input_name + '" cols="34" rows="5" class="form-control notranslate">' + value + '</textarea>');
						output.push('</li>');
						output.push('</ul>');
						break;
					case 'number':
		...
		for (var r = 0; r < question.buttons.length; r+=1) {
			...
			if (button_type === "exit") {
				...
				output.push('<div class="row question-footer">');
					output.push('<div id="exit-interview" class="' + button_type + '-button pull-right btn btn-danger notranslate">Exit</div>');
				output.push('</div>');
			} else if (button_type === "finish") {
				// removes the button event listener
				output.push('<div class="row question-footer">');
					output.push('<div id="finish-interview" class="' + button_type + '-btn button btn btn-success pull-right notranslate">Finish</div>');
				output.push('</div>');
			} else {
				output.push('<div class="row question-footer">');
					output.push('<div class="' + button_type + '-button button btn btn-primary pull-right notranslate" data-loading-text="Loading..." autocomplete="off" data-qid="' + question.qid + '">' + button_type.charAt(0).toUpperCase() + button_type.slice(1) + ' <i class="fa fa-arrow-circle-o-right"></i></div>');
				output.push('</div>');
			}
		}
		output.push('<div class="clear"></div>');
