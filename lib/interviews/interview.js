 ...
 * @data {object} From the database so we can use it user defined functions in the interview
 *
 */
function build (question, master, logic_debug, type, fields, data) {
	"use strict";
	...
	var date_pickers = [];
	var max, min, values, field, label_class, input_name, input_type, input_label, input_default, input_validation, input_values, value, button, button_type;
	...
	if (question.learn_more) {
		if (question.learn_more.body) {
			output.push('<div class="learnmore">');
				output.push('<div class="lm-display">');
					output.push('<div class="lm-icon">?</div>');
				output.push('</div>');
				output.push('<div class="lm-data">');

					if (question.learn_more.title) {
						output.push('<div class="lm-title-sm">'); 
						output.push(helpers.parseText(question.learn_more.title, master, data));
						output.push('</div>'); 
					}

					output.push('<div class="lm-body-sm">'); 
					output.push(helpers.parseText(question.learn_more.body, master, data));
					output.push('</div>'); 
				output.push('</div>');
			output.push('</div>');
		}
	...
	if (question.fields) {
		for (var i = 0; i < question.fields.length; i+=1) {
			...
			if (input_name && input_type) {
				...
				output.push('<div id="' + input_name + '-var-container" class="var-container">');//use this to attach error class
				...
				switch (input_type) {
					...
					case 'radio':
						if (input_values) {
							value = (type === 'back') ? helpers.getAnswer(input_name, fields) : "";
					...
					case 'date':
						...
						value = (type === 'back') ? helpers.getAnswer(input_name, fields) : null;
						output.push('<ul id="' + input_name + ':' + input_type + '" class="id-field">');
					...
					case 'checkbox':
						if (input_values) {
							...
							if (type === 'back') {
								values = helpers.getAnswer(input_name, fields);
							}
	...
	if (question.buttons) {
		for (var r = 0; r < question.buttons.length; r+=1) {
			...
			if (button_type === "exit") {
				...
				output.push('<div class="' + button_type + '-button button notranslate">' + question.qid + '</div>');
			}
		}
		output.push('<div class="clear"></div>');
	}
					...
					for (var j = 0; j < item.actions.length; j+=1) {
						...
						if (act.action === 'goto' && !skip_goto) {
							...
							if (act.if === condition) {
								...
								if (check_var.name) {
									// check if the set var name is an array nam[loop] or just name
...
exports.next = function (qid, question, data, master, fields, progress, state) {
	...
	var run_build = build(data[before_logic.qid], before_logic.vars, before_logic.debug.join(""), 'next', fields, data);
