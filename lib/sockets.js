	...
	io.sockets.on('connection', function (client) {
		...
		client.on('start', function (data) {
			...
			models.Interviews.findOne({id: data.interview_id}, function (err, doc) {
				...
				if (!doc) {
					console.log('Interview could not be found!');
					throw err;
				}
				var qid = doc.start;
				...
				models.Counters.findOne({}, function (err, counter) {
					...
					models.Counters.update({tmp_count: tmp_count, state_count: state_id}, function (err) {
						...
						if (app.get('disable_graphviz_progress')) {
							...
							// we might not need to update the progress fraction if the interview has not been modified
						...
						models.Counters.findOne({}, function (err, counter) {
							...
							models.Counters.update({state_count: new_state_id}, function (err) {
								...
								if (validate.error) {
									...
									if (loop) {
										for (var prop in connected_clients[id].data.master.vars) {
											if (connected_clients[id].data.master.vars.hasOwnProperty(prop)) {
												if (connected_clients[id].data.master.vars[prop].loop) {
													if (connected_clients[id].data.master.vars[prop].qid === qid) {
													    if (Array.isArray(connected_clients[id].data.master.vars[prop].values[prop])) {
													    	loop_index = connected_clients[id].data.master.vars[prop].values[prop].length;
													    	break;
													    } 
													}
												}
									...
									// ** We need a better way to get the loop_index, they are using the next question 
									...
									if (next_loop) {
										...
										for (var prop in connected_clients[id].data.master.vars) {
											if (connected_clients[id].data.master.vars.hasOwnProperty(prop)) {
												if (connected_clients[id].data.master.vars[prop].loop) {
													if (connected_clients[id].data.master.vars[prop].qid === next.qid) {
													    if (Array.isArray(connected_clients[id].data.master.vars[prop].values[prop])) {
													    	next_loop_index = connected_clients[id].data.master.vars[prop].values[prop].length;
													    	break;
													    } 
													}
												}
									...
									models.States.find(query).sort({created: 1}).exec(function (err, states) {
										...
										fraction = connected_clients[id].data.distance[run.qid];

										// update master vars list, since new answers could be created in the logic
										connected_clients[id].data.master.vars = run.master; 

										// is we are in a loop we need to attach the new loop index (based on the values array)
										// if (loop) {
										// 	// grab the first field, any would do since their values array will all be equal lengths
										// 	for (var prop in connected_clients[id].data.master.vars) {
										// 	    if (Array.isArray(connected_clients[id].data.master.vars[prop].values[prop])) {
										// 	    	loop_index = connected_clients[id].data.master.vars[prop].values[prop].length;
										// 	    	break;
			...
			models.Interviews.findOne({ id: data.interview }, function (err, doc) {
				...
				models.Tmps.findOne({ id: data.id }, function (err, tmp) {
					...
					models.States.find({id: { $in: [history.new_current_state, history.removed_state]} }).sort({id: -1}).exec(function (err, states) {
						...
						// find out the fractional progress of the interview
						fraction = connected_clients[id].data.distance[back.qid];				
							...
							models.Counters.update({tmp_count: tmp_count}, function (err) {
								...
								var send_data = {
									...
									data: {
										...
										debug: null,
										fraction: partial.data.state.distance[partial.qid]
									},
									valid: true,
									partial:true
								};
						...
						models.Counters.findOne({}, function (err, counter) {
							...
							if (validate.error) {
								...
								if (deliverables.length !== 0) {
									// BUILD the final output..this callback gets fired when all the deliverables have been created
									process.output(doc, connected_clients[id].data.master.vars, tmp_history, base_location, app, function (err, data) {
										if (err) {
											// if any stylesheet was not procure send back some error to the client
											console.log(err);
											send_data = {
												id: tmp.id,
												error: interview.error().error.content,
												valid: false
											};  
											...
											if (on_complete.email_deliverables_to_client) {
												...
												// this is the data that gets sent back to the client. It involves the HTML of the question to show the user, and debug info for the editor (if edit mode)
												send_data = {
													id: tmp.id,
													qid: run.qid,
													data: {
									...
									send_data = {
										id: tmp.id,
										error: '<div class="contents"><div class="prompt"><div class="text"><p>The interview completed successfully, however, no deliverables are set for this interview.</p></div></div></div>',
										valid: false
									};  
									emitData(id, 'srv_error', send_data);   
								}
		...
		client.on('disconnect', function () {
			/*
			// need to find what socket disconnected by finding where the id is in the array
			var index = utils.findIndex('id', client.id, connected_clients[id].sockets);

			if (index >= 0) {
				// get rid of the socket
				connected_clients[id].sockets.splice(index,1);
				console.log('   debug - ' + 'socket ' + client.id + 'is closed');
			}
			

			//since the client could be connected to both the viewer, and editor we cant just delete this
			connected_clients[id].data = {
				progress: [],
				master: [],
				deliverables: {},
				client: null,
			...
			// if all the sockets close, (viewer and editor), delete the client
			if (connected_clients[id].sockets.length === 0) {
				delete connected_clients[id];
				console.log('   debug - ' + 'client ' + id + 'is deleted');
			}
