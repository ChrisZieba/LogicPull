						...
						models.Counters.findOne({}, function (err, counter) {
							...
							models.Counters.update({state_count: new_state_id}, function (err) {
								...
								if (validate.error) {
									...
									// if the question has been viewed before uses the latest states answers to prepopulate the question
...
// console.log("next:" + next.qid);
// console.log("tmp:" + tmp.id);
									// Look for any states that have run this question already
									models.States.find({tmp_id: tmp.id, base_qid: next.qid}).sort({created: -1}).exec(function (err, states) {
