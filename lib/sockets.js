...
exports.listen = function (server, sessionStore, app) {
	...
	io.sockets.on('connection', function (client) {
		...
		if ( ! connected_clients.hasOwnProperty(client.id)) {
			...
			console.log('   debug - ' + 'client ' + id + 'is connected');
			...
			models.Interviews.findOne({id: data.interview_id}, function (err, doc) {
				...
				models.Counters.findOne({}, function (err, counter) {
					...
					var startInterview = function (graph) {
						...
						// we need to save the order the questions are answered   
					...
					models.States.findOne({id: tmp.current }, function (err, state) {
						...
						models.Counters.findOne({}, function (err, counter) {
							...
							if (validate.error) {
								...
								// now that we know the answers are valid, we can run any after logic in th question,and before logic in the destination
								run = interview.next(qid, doc.data[qid], doc.data, connected_clients[id].data.master.vars, fields, tmp.history, new_state_id);
		...
		client.on('graph', function (data) {
			if (app.get('disable_graphviz_tidy')) {
				...
				g.output(options, "dot", function (out) { 
