...
exports.clean = function (str, is_image) {
	...
	for (var r in never_allowed_str) {
		str = str.replace(new RegExp(r, "gi"), never_allowed_str[r]);
	}

	// Remove regex patterns that are never allowed
	for (var j in never_allowed_regex) {
		str = str.replace(new RegExp(j, 'gi'), never_allowed_regex[j]);
	}


	var concactify = function (m, compat, after) {
		return compat.replace(/\s+/g, '') + after;
	};

	// Compact any exploded words like:  j a v a s c r i p t
	// We only want to do this when it is followed by a non-word character	
	for (var k = 0, l = compact_words.length; k < l; k++) {
		var spacified = compact_words[k].split('').join('\\s*')+'\\s*';

		str = str.replace(new RegExp('(' + spacified + ')(\\W)', 'ig'), concactify);
	}

	var replace_links = function (m, attributes, end_tag) {
	...
	var replace_img = function (m, attributes, end_tag) {
		attributes = filter_attributes(attributes.replace('<','').replace('>',''));
	...
	do {
		if (str.match(/<a/i)) {
			str = str.replace(/<a\s+([^>]*?)(>|$)/gi, replace_links);
		}

		if (str.match(/<img/i)) {
			str = str.replace(/<img\s+([^>]*?)(\s?\/?>|$)/gi, replace_img);
		}
	...
	for (var t in never_allowed_str) {
		str = str.replace(new RegExp(t, "gi"), never_allowed_str[t]);
	}
	for (var m in never_allowed_regex) {
		str = str.replace(new RegExp(m, 'gi'), never_allowed_regex[m]);
	}
...
function remove_invisible_characters (str) {
	for (var n = 0, o = non_displayables.length; n < o; n++) {
		str = str.replace(non_displayables[n], '');
	}
	return str;
}
