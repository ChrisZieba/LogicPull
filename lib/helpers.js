...
function evaluate (condition, vars, type, data) {
  ...
  fn.fieldLabel = function (name, id, index) {
    index = (typeof index !== 'undefined') ? parseInt(index, 10) : null;

    if (vars.hasOwnProperty(name)) {
      var qid = vars[name].qid;
      for (var i = 0; i < data[qid].fields.length; i+=1) {
        // Avoid text variables when lookiung up the field
        if (data[qid].fields[i].name === name && data[qid].fields[i].type !== 'text') {
          // Check if the id given is in the id's of the field
          for (var j = 0; j < data[qid].fields[i].values.length; j+=1) {
            if (data[qid].fields[i].values[j].id === id) {
              return data[qid].fields[i].values[j].label;
            }
          }
        }
      }
          ...
          if (loop) {
            if (master[loop_name] !== null && typeof master[loop_name] !== 'undefined') {
              if (k < master[loop_name].values[loop_name]) {
                // pad with null. The reason for this is so when referencing array values they match up.
...
exports.numberCheck = function (val) {
  ...
  if (val !== null && typeof val !== 'undefined' && val !== '') {
    ...
    // If the answer is a number (contains only digits) then parse the integer
    if (isnum) {
      val = parseFloat(val);
      type = 'number';
    }      
...
exports.parseText = function (text, vars, data) {
  ...
  text = text.replace(hre, function (match, identifier, hidden) {
    ...
    modal.push('<a href="#" class="help-modal" data-toggle="modal" data-target="#help-modal-' + count + '">');
      modal.push(identifier);
    modal.push('</a>');

    modal.push('<div class="modal fade" id="help-modal-' + count + '" tabindex="-1" role="dialog" aria-hidden="true">');
      modal.push('<div class="modal-dialog">');
        modal.push('<div class="modal-content">');
          modal.push('<div class="modal-header">');
            ...
            modal.push('<h4 class="modal-title">' + identifier + '</h4>');
          modal.push('</div>');
          modal.push('<div class="modal-body">');
            modal.push(hidden);
          modal.push('</div>');
          modal.push(' <div class="modal-footer">');
            modal.push('<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>');
          modal.push('</div>');
        modal.push('</div>');
      modal.push('</div>');
    modal.push('</div>');
  ...
  text = text.replace(yre, function (match, identifier, hidden) {
    ...
    modal.push('<a href="' + hidden + '" class="youtube-modal" data-target="#youtube-modal-' + count + '">');
      modal.push(identifier);
    modal.push('</a>');

    modal.push('<div class="modal fade" id="youtube-modal-' + count + '" tabindex="-1" role="dialog">');
      modal.push('<div class="modal-dialog modal-lg">');
        modal.push('<div class="modal-content">');
          modal.push('<div class="modal-header">');
            modal.push('<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>');
            modal.push('<h4 class="modal-title">' + identifier + '</h4>');
          ...
          modal.push('<div class="modal-body">');
            ...
            modal.push('<div class="embed-responsive embed-responsive-16by9"></div>');
          modal.push('</div>');
          modal.push(' <div class="modal-footer">');
            modal.push('<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>');
          modal.push('</div>');
 ...
 *
 */
exports.getAnswer = function (name, fields, type, def, answers) {
  "use strict";

  // set the answer to the input default
  var answer = def;

  // if the prev_answer is set, it ised used
  if (answers !== null && typeof answers !== 'undefined') {
    if (answers.hasOwnProperty(name)) {
      var values = answers[name].values[name];
      if (Array.isArray(values)) {
        answer = values[values.length-1];
      } else {
        answer = values;
      }
    }
